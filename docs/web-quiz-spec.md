...
- Save each reached round immediately as `Not answered`, including its study/briefing phase, and replace that same session/round record on submission or timeout. Refresh retains completed results and the current unanswered round, never unreached future rounds. Repeated refreshes must not duplicate records. `Not answered` is a separate history/CSV outcome and counts against accuracy. Save at round transitions rather than relying on unreliable unload events; existing history remains intact.
- The single optional auto-continue setting advances immediately after either a valid submitted answer (correct or incorrect) or timeout in all four modes. It opens the next mode's normal study/briefing/question phase, or the summary after the last round. Invalid/unsubmitted forms do not advance. When unchecked, retain feedback and manual Next.
